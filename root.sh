...
############################
# INSTALL UBUNTU 24.04 (FIXED 404)
############################

install_ubuntu() {
# 永久可用官方镜像
UBUNTU_URL="https://cdimage.ubuntu.com/ubuntu-base/releases/24.04/release/ubuntu-base-24.04.4-base-${ARCH_ALT}.tar.gz"
...
wget --tries="$MAX_RETRIES" --timeout="$TIMEOUT" --show-progress --no-hsts -O "$ROOTFS_DIR/usr/local/bin/proot" "https://proot.gitlab.io/proot/bin/proot"
...
apt update -y
apt install -y sudo curl wget nano vim git htop neofetch net-tools openssh-server ca-certificates software-properties-common zip unzip screen tmux python3 python3-pip build-essential
...
safe_var() {
VALUE="$1"
if [ -z "$VALUE" ]; then echo "Not Available"; else echo "$VALUE"; fi
}
